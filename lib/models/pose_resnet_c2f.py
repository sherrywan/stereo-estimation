...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, num_joints,
        ...
        if alg_confidences:
            self.alg_confidences = GlobalAveragePoolingHead(
                512 * block.expansion, num_joints)

        if vol_confidences:
            self.vol_confidences = GlobalAveragePoolingHead(
                512 * block.expansion, 32)

        # used for deconv layers
        self.deconv_layers = []
        if self.num_deconv_layers == 1:
            self.num_deconv_filters = [self.num_deconv_filters]
            self.num_deconv_kernels = [self.num_deconv_kernels]
            
        for i in range(self.num_deconv_layers):
            self.deconv_layers.append(self._make_deconv_layer(
                1,
                self.num_deconv_filters[i],
                self.num_deconv_kernels[i],
            ))
...
def get_pose_net(config, device='cuda:0'):
    ...
    if config.init_weights:
        ...
        for k, v in pretrained_state_dict.items():
            if k.replace(prefix, "") in model_state_dict and v.shape == model_state_dict[k.replace(prefix, "")].shape:
            ...
            elif "deconv_layers" in k.replace(prefix, ""):
                k = k.replace(prefix, "")
                if k in model_state_dict and v.shape == model_state_dict[k].shape:
                    ...
                    layer_old = k[14]
                    if layer_old=='0' or layer_old=='1':
                        k = k.replace(".0", ".0.0")
                        k = k.replace(".1", ".0.1")
                    elif layer_old=='3' or layer_old=='4':
                        k = k.replace(".3", ".1.0")
                        k = k.replace(".4", ".1.1")
                    elif layer_old=='6' or layer_old=='7':
                        k = k.replace(".6", ".2.0")
                        k = k.replace(".7", ".2.1")
                    if k in model_state_dict and v.shape == model_state_dict[k].shape:
                        new_pretrained_state_dict[k] = v
        ...
        not_inited_params = set(map(lambda x: x.replace(
            prefix, ""), pretrained_state_dict.keys())) - set(new_pretrained_state_dict.keys())
