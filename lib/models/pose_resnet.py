...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, num_joints,
        ...
        if self.final_conv_kernel == 3:
            final_padding = final_dilation
        else: 
            final_padding = 0
        ...
        if mask_type == "concat":
            ...
            self.final_layer_concat = nn.Conv2d(
    ...
    def forward(self, x):
        ...
        x_1 = self.final_layer(x)
        heatmaps = x_1
        
        if self.final_concat:
            x_2 = self.final_layer_1(x)
            x_3 = self.final_layer_2(x)
            x_4 = x_1 + x_2 + x_3
            x_4 = self.final_layer_concat(x_4)
            heatmaps = x_4

        return heatmaps, features, alg_confidences, vol_confidences, x_1, x_2, x_3
...
def get_pose_net(config, device='cuda:0'):
    ...
    model = PoseResNet(
        ...
        final_dilation = config.final_dilation,
        mask_type= config.mask_type if hasattr(config, "mask_type") else ""
    )
