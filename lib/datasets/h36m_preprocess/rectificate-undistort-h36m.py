"""
    ...
    Usage: `python3 undistort-h36m.py <path/to/Human3.6M-root> <path/to/human36m-stereo-labels.npy> <num-processes>`
"""
...
number_of_processes = int(sys.argv[4])
...
dataset = Human36MMultiViewDataset(
    ...
    kind="mpii",
    norm_image=False,
    undistort_images=True,                 # don't do unnecessary image processing
...
def rectificate_and_undistort_and_save(idx):
    sample = dataset[idx]
    ...
    for camera_idx, bbox in enumerate(shot['bbox_by_camera_tlbr']):
        if bbox[2] == bbox[0]: # bbox is empty, which means that this camera is missing
            ...
            print("remove {} {} {} {} ".format(subject, action, camera_idx, frame_idx+1))
...
for _ in tqdm(pool.imap_unordered(
    rectificate_and_undistort_and_save, range(len(dataset)), chunksize=1), total=len(dataset)):
    pass
