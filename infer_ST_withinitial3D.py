...
from lib.datasets import mhad_stereo_for_refine, human36m_for_MaskST
...
def setup_mhad_dataloaders(config, is_train, distributed_train):
    ...
    if is_train:
        # train
        train_dataset = mhad_stereo_for_refine.MHADStereoViewDataset(
        ...
        train_dataloader = DataLoader(
            ...
            collate_fn=make_collate_fn(randomize_n_views=config.dataset.train.randomize_n_views,
    ...
    # val
    val_dataset = mhad_stereo_for_refine.MHADStereoViewDataset(
        mhad_root=config.dataset.train.mhad_root,
    ...
    val_dataloader = DataLoader(
        ...
        collate_fn=make_collate_fn(randomize_n_views=config.dataset.val.randomize_n_views,
...
def prepare_batch_for_pretrain(batch, device, config, is_train=True):
    ...
    return keypoints_3d_batch_pred, keypoints_3d_batch_gt, keypoints_3d_validity_batch_gt


def one_epoch(criterion, config, dataloader, device, epoch, refine_model=None, mask_ratio=0, opt_refine=None, n_iters_total=0, is_train=True, caption='', master=False, experiment_dir=None, writer=None, visible=False, dataset_type= 'mhad', method = "ltrialg"):
    ...
    with grad_context():
        ...
        for iter_i, batch in iterator:
            with autograd.detect_anomaly():
                ...
                keypoints_3d_batch_pred, keypoints_3d_batch_gt,  keypoints_3d_validity_gt = prepare_batch_for_pretrain(
                    batch, device, config)

                batch_size = keypoints_3d_batch_pred.shape[0]
                n_joints = keypoints_3d_batch_pred.shape[1]

                if config.model.transformer_refine.relative_pose:
                    keypoints_root_pred = keypoints_3d_batch_pred[:,6:7,:].clone()
                    keypoints_3d_pred = keypoints_3d_batch_pred - keypoints_root_pred
                    keypoints_root_gt = keypoints_3d_batch_gt[:,6:7,:].clone()
                    keypoints_3d_gt = keypoints_3d_batch_gt - keypoints_root_gt

                keypoints_3d_binary_validity_gt = (
                    keypoints_3d_validity_gt > 0.0).type(torch.float32)
                
                if refine_model is not None:
                    keypoints_3d_pred_refined, _ = refine_model(keypoints_3d_pred, None)
                ...
                if config.model.transformer_refine.relative_pose:
                    ...
                    keypoints_3d_pred_refined = keypoints_3d_pred_refined + keypoints_root_pred
                # save answers for evalulation
                if not is_train:
                    results['keypoints_3d'].append(
                        keypoints_3d_pred_refined.detach().cpu().numpy())
                    results['keypoints_3d_initial'].append(
                        keypoints_3d_batch_pred.detach().cpu().numpy())
    ...
    if master:
        if not is_train:
            ...
            np.save(os.path.join(experiment_dir, "keypoints_3d_refined_{}.npy".format(method)),
                    results['keypoints_3d'])
            results['keypoints_3d_initial'] = np.concatenate(
                results['keypoints_3d_initial'], axis=0)
            results['keypoints_3d_gt'] = np.concatenate(
                results['keypoints_3d_gt'], axis=0)
            results['indexes'] = np.concatenate(results['indexes'])

            try:
                scalar_metric, _, full_metric = dataloader.dataset.evaluate(
                    results['keypoints_3d'])
                scalar_metric_1, _, full_metric_1= dataloader.dataset.evaluate(
                    results['keypoints_3d_initial'])
            ...
            with open(os.path.join(checkpoint_dir, "metric_initial.json".format(epoch)), 'w') as fout:
                json.dump(full_metric_1, fout, indent=4, sort_keys=True)
...
def main(args):
    ...
    method = os.path.basename(config.dataset.val.pred_results_path).split('_')[-2]
    ...
    if master:
        experiment_dir, writer = setup_experiment(
            config, method, is_train=not args.eval)
    ...
    if not args.eval:
        ...
        for epoch in range(config.opt.n_epochs):
            ...
            n_iters_total_train, _ = one_epoch(criterion, config, train_dataloader_mhad, device, epoch, refine_model=refine_model_train, opt_refine=opt_refine, 
            ...
            n_iters_total_val, mpjpe = one_epoch(criterion, config, val_dataloader_mhad, device, epoch, refine_model=refine_model_eval, opt_refine=opt_refine,
            ...
            print(f"The best results is in {epoch_min}th epoch with {mpjpe_min} mpjpe!")
        ...
        if args.eval_dataset == 'train':
            ...
            n_iters_total_val, mpjpe = one_epoch(criterion, config, val_dataloader_mhad, device, 0, refine_model=refine_model_eval, opt_refine=opt_refine,
                                          n_iters_total=0, is_train=False, master=master, experiment_dir=experiment_dir, writer=writer, method=method)
