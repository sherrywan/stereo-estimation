...
def one_epoch(criterion, config, dataloader, device, epoch, refine_model=None, mask_ratio=0, opt_refine=None, n_iters_total=0, is_train=True, caption='', master=False, experiment_dir=None, writer=None, visible=False, dataset_type= 'mhad'):
        ...
        for iter_i, batch in iterator:
            with autograd.detect_anomaly():
                ...
                if refine_model is not None:
                    keypoints_3d_pred_refined = refine_model(keypoints_3d_pred, None)
...
def main(args):
    ...
    if master:
        experiment_dir, writer = setup_experiment(
            config, "MaskST", is_train=not args.eval)
    ...
    if not args.eval:
        ...
        for epoch in range(config.opt.n_epochs):
            ...
            n_iters_total_train, _ = one_epoch(criterion, config, train_dataloader_h36m, device, epoch, refine_model=refine_model_train, opt_refine=opt_refine, mask_ratio=p,
                                            n_iters_total=n_iters_total_train, is_train=True, master=master, experiment_dir=experiment_dir, writer=writer, dataset_type="h36m")
            print("Start evaluating in h36m!")
            refine_model_eval.load_state_dict(
                refine_model_train.state_dict(), strict=False)
            n_iters_total_val, _ = one_epoch(criterion, config, val_dataloader_h36m, device, epoch, refine_model=refine_model_eval, opt_refine=opt_refine, mask_ratio=p,
                                          n_iters_total=n_iters_total_val, is_train=False, master=master, experiment_dir=experiment_dir, writer=writer, dataset_type="h36m")
            ...
            refine_model_eval.load_state_dict(
                refine_model_train.state_dict(), strict=False)
            n_iters_total_val, mpjpe = one_epoch(criterion, config, val_dataloader_mhad, device, epoch, refine_model=refine_model_eval, opt_refine=opt_refine, mask_ratio=p,
